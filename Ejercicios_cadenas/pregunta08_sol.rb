...
def obtenerLetraDesplazada(abecedario,letra,desplazamiento)
	...
	for i in 0..abecedario.size-1
		if abecedario[i] == letra
		    indice = i + desplazamiento
			if indice > abecedario.size-1
				letraDesplazada = abecedario[indice - abecedario.size] 
			else
				letraDesplazada = abecedario[indice]
			end
		end
	end	
...
def cifrado(texto,desplazamiento,abecedario)
	...
	for i in 0..texto.size-1
		nuevotexto = nuevotexto + obtenerLetraDesplazada(abecedario, texto[i], desplazamiento).to_s 
	end
	return nuevotexto
end
